 ...
 private:
  ...
  static bool IsWhitelisted(const std::string& path) {
    ...
    if (access("/system/framework/XposedBridge.jar", F_OK ) != -1 ||
        access("/magisk/xposed/system/framework/XposedBridge.jar", F_OK ) != -1) {
      ...
      static const std::string kSystemAppPrefix = "/system/app/";
      if (path.compare(0, kDataAppPrefix.size(), kDataAppPrefix) == 0 ||
          path.compare(0, kSystemAppPrefix.size(), kSystemAppPrefix) == 0) {
        return true;
      }
    }
